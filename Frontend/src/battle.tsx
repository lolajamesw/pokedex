...
export default function PokemonCounterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch("/api/pokemon-counter", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
        body: JSON.stringify({ pokemonName: pokemonName.trim() }),
      })
  ...
  return (
    <div className="container mx-auto px-4 py-8 max-w-4xl">
      <div className="text-center mb-8">
        <h1 className="text-4xl font-bold mb-2 text-blue-600">Pokemon Counter Finder</h1>
        ...
        <div className="space-y-6">
          <div className="text-center">
            <h2 className="text-2xl font-semibold mb-2">
              Best Counters for <span className="text-blue-600 capitalize">{results.targetPokemon}</span>
            </h2>
              ...
              <Card key={pokemon.id} className="hover:shadow-lg transition-shadow">
                <CardHeader className="pb-3">
                  ...
                  <div className="flex gap-2">
                    <Badge className={`${getTypeColor(pokemon.type1)} text-white`}>{pokemon.type1}</Badge>
                    {pokemon.type2 && (
                      <Badge className={`${getTypeColor(pokemon.type2)} text-white`}>{pokemon.type2}</Badge>
                    )}
                  </div>
                </CardHeader>
