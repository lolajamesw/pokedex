...
SELECT atk.type, atkSum, defSum FROM (
   SELECT type, SUM(effect)/(SELECT COUNT(*) FROM MyPokemon WHERE uID=5 AND onteam=1) as defSum FROM (
	SELECT type1 type, effect*(
		SELECT COUNT(*) from Pokedex p WHERE pID IN (
			SELECT pID FROM MyPokemon WHERE uID=5 AND onteam=1
		)
...
) as def, (SELECT type, SUM(effect)/(SELECT COUNT(*) FROM MyPokemon WHERE uID=5 AND onteam=1) 
     as atkSum FROM (
